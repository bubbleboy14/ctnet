CAN.media.video = {
	...
	"getAndShow": function(vid) {
		...
		if (viddata && viddata.player)
			...
			CAN.widget.conversation.jump(vid, "video", function(result, comm) {
				var vnode = CAN.media.video.build(result,
					null, true, true, true).firstChild;
				(CT.dom.id(n.id) || n).innerHTML = (comm ? CT.dom.div(CT.dom.div([
					CAN.widget.conversation.bare(comm),
					CT.dom.link("from thread", null,
						"/video.html#!" + CAN.cookie.flipReverse(comm.key),
