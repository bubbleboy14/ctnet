CAN.media.news = {
	...
	"build": function(d, vindex, howmuch, nocat, v) {
	    ...
	    var n = howmuch == "slider" && CT.dom.node() || CT.dom.node("", "div",
	    ...
	    if (howmuch == "full" || howmuch == "intro") {
	        if (howmuch == "intro")
	            n.appendChild(CT.dom.node(CT.dom.link(d.title, function() {
	                v.newMediaViewMoreCb(d); }), "div", "newstitletext"));
	        else
	            n.appendChild(CT.dom.node(d.title, "div", "big bold red"));
	        ...
	        var byline = CT.dom.node("", "div", "newstitle small italic");
	        byline.appendChild(CT.dom.node("posted " + (d.date
	        	|| "(now)") + ", by ", "span", "gray"));
	    ...
	    if (d.photo.length > 0) {
	        if (howmuch == "full") {
	            ...
	            n.appendChild(CAN.media.photo.viewSingle(CT.data.get(d.photo[0]),
	                ...
	                : (howmuch == "intro") ? "right newsimg"
	                : "right lmargimg", function() { v.newMediaViewMoreCb(d); }));
	        }
	    }

	    if (howmuch == "teaser" || howmuch == "result" || howmuch == "slider") {
	        var tnode = CT.dom.node(d.title, "div",
	...
	//    var b = processTemplate(d.body);
	    ...
	    if (howmuch == "full") {
	        ...
	        // body
	        n.appendChild(CT.dom.node(b, "div", "newsbodytext"));
	        //categories
	        if (!nocat)
	        ...
	        for (var i = 0; i < d.video.length; i++)
	            n.appendChild(CAN.media.video.build(CT.data.get(d.video[i]),
	                null, true));
	        if (!nocat) {
	            var convonode = CT.dom.node("loading conversation...", "div", "red bordertop");
	        ...
	        var nbt = CT.dom.node(itxt, "div", "newsbodytext");
