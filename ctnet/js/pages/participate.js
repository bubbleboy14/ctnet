    ...
    loaders["videographer"] = function() {
        ...
        var majorVideoData = function() {
            ...
            if (d && CT.video.rawVidTypes.indexOf(d.player) == -1)
                return d;
            alert('No usable value for "link". Please note that while we support embedded video from DTube, BitChute, Rumble, Odysee, lbryplayer, UGETube, GabTV, Vimeo, YouTube (ew), Google Video (ew), Facebook (ew), and uStream, as well as links to raw files (mp4, ogg, webm, and mov), our "Video Adder" (this thing) can only handle Rumble, DTube, Vimeo, YouTube (ew), and Google Video (ew). We\'re always working to improve the site, so check back soon!');
        };
